...
describe("listFiles", () => {
  ...
  it("streams hashes that are byte-identical to the full-buffer SHA-256 SRI", async () => {
    ...
    // `sriSha256` here is the reference full-buffer implementation
    // (`createHash().update(entireBuffer).digest("base64")`). The streamed hash
    // computed by the store's `hashFileStream` must match it exactly — this is
    // the RFC G-5 byte-identical guarantee the streaming refactor must preserve.
    ...
    // Sizes still come from stat and must be unaffected by how we hash.
