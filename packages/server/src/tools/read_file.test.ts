...
describe("read_file tool", () => {
  ...
  // ── kitId cross-kit isolation (DRO-509 security fix, ported from PR #116) ──
  it("rejects an empty kitId instead of reading across sibling kits", async () => {
    // With an empty kitId, the store's kitDir would resolve to the kits root
    // itself, so a relative `path` of `other-kit/secret.txt` would escape
    // `test-kit` and read a sibling kit's file. The shared `isSafeKitId` guard
    // (schema `.min(1)` + handler check) must reject it before that resolution.
