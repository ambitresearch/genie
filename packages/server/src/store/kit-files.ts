...
/**
 * The ONE kitId-safety rule shared by the kit-taking tools that apply it and
 * both `KitStore` adapters, so their defenses cannot silently drift apart
 * (DRO-509 / DRO-581 unification). `validate` does not apply this input gate,
 * but accepts every id that passes it; its full-scan path reaches adapters that
 * enforce the same rule. Lives here — not in a tool module — because the
