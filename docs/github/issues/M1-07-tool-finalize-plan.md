---
title: "[M1-07] Tool: finalize_plan (capability grant boundary)"
...
## Summary
Implement `finalize_plan` — the **single user-visible permission grant** that
...
- Research report §3.1 + §1.3: "Plan boundary (the single permission grant) —
  finalize_plan — locks `writes` (max 256 glob patterns, ≤3 wildcards each),
  `deletes`, `localDir`".
...
## Acceptance Criteria
- [ ] AC1 — Tool name `mcp__genie__finalize_plan`.
- [ ] AC2 — Input: `{ projectId: string, writes: string[], deletes?: string[],
      localDir?: string }`.
...
- [ ] AC8 — Plan state is persisted to
      ...
      restart.
- [ ] AC9 — Concurrent `finalize_plan` calls on the same `projectId` from
      different sessions are allowed (each gets its own `planId`).
- [ ] AC10 — Emits a `plan.created` audit log line with `{ projectId, planId,
      writeCount, deleteCount }` (no path contents).

## Implementation Notes
- File: `packages/server/src/tools/finalize_plan.ts` + `src/plans/`.
...
## Dependencies
- Blocks: M1-08, M1-09, M1-10, M1-11, M1-12.
- Blocked by: M1-06.
