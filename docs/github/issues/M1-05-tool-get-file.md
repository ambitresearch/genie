---
title: "[M1-05] Tool: get_file (with 256 KiB cap)"
...
## Summary
Implement `get_file` — returns the content of a single file. Hard-capped at
256 KiB per DesignSync's contract. The skill only ever uses it to fetch
`_ds_sync.json` as the verification anchor, but the verb must be general.

## Context
- Research report §3.1: `get_file({ projectId, path }): { content: string }
  // 256 KiB cap`.
- Research report §1.4 confirmed claim: "get_file — capped at 256 KiB; the
  skill only ever uses it to fetch `_ds_sync.json` as the verification
  anchor".

## Acceptance Criteria
- [ ] AC1 — Tool name `mcp__genie__get_file`.
- [ ] AC2 — Input: `{ projectId: string, path: string }`.
...
- File: `packages/server/src/tools/get_file.ts`.
