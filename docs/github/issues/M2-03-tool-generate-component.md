---
title: "[M2-03] Tool: generate_component"
milestone: "M2 — LiteLLM Generation Surface"
labels: ["type:feature", "area:mcp-tools", "area:litellm", "priority:P0-critical", "size:L"]
assignees: []
estimate: "12h"
---
...
## Acceptance Criteria
- [ ] AC1 — Tool name `mcp__genie__generate_component`.
- [ ] AC2 — Input: `{ projectId: string, kit: string, prompt: string, group?:
...
- [ ] AC3 — Default `framework` = `"react"`; default `model` =
      `"design-default"` (resolves to `anthropic/claude-sonnet-4-6` via
      LiteLLM alias).
...
- File: `packages/server/src/tools/generate_component.ts`.
...
**Primary mock:** [`00-front-door.svg`](https://github.com/roshangautam/genie/blob/main/docs/designs/design-1/00-front-door.svg) — the GENERATE moment — accent applies here.
...
## Definition of Done
- [ ] Tests added — unit (stub LLM) + integration (real LiteLLM with $5 cap).
- [ ] Docs updated.
