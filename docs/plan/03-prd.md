...
| Design lead      | TBD                                                                                                     |
| Source of truth  | `genie/.deliverables/research-report.json` (46-agent custom workflow, 19/20 claims verified) |
| Related docs     | `docs/01-product-vision.md`, `docs/02-brd.md`, `docs/04-tech-design-rfc.md`, `github/milestones.md`     |
| Repo             | `roshangautam/genie` (private, MIT, to be created)                                       |
...
**genie** is a harness-agnostic, open-source MCP server inspired by Anthropic's
Claude Design — an independent take on the same idea, delivered as a single TypeScript MCP server
...
LiteLLM-routed model access, run their own homelab or VPC infrastructure, or
need to integrate a design-system feedback loop into non-Anthropic harnesses
(Codex CLI, Cursor, Cline, Continue) have no native way to consume Claude
Design today. This product fills that gap.

genie mirrors three load-bearing mechanics from the same protocol — (1) the 12-method
DesignSync verb surface with its strict `read → finalize_plan → write/delete`
capability boundary, (2) the first-line `<!-- @dsCard group="…" -->` HTML marker
that registers preview cards in the Design System pane, (3) the 5-step atomic
write sequence ending with a `_ds_sync.json` verification anchor — and adds a
portable preview pane that ships in three delivery vehicles from a single artifact
set: `file://` (universal), `http://localhost:5173` (Vite-backed live viewer with
HMR, universal), and `ui://genie/grid` (MCP-UI Apps payload for
Claude/VS Code/ChatGPT/Cursor inline rendering).

Generation is routed through the user's existing LiteLLM gateway
(`https://litellm.roshangautam.com`) so model choice (Sonnet 4.6, Opus 4.8,
Qwen3-Coder local), budgets, rate limits, and observability all reuse the
gateway's existing per-key controls rather than reinventing them. Distribution
...
for a team, and OSS plugin authors who want to integrate a design-system
...
| Success indicators   | Generation P50 < 4s end-to-end; viewer first-paint < 500ms for 100 components; zero `@dsCard` regressions across a 30-day window; ≥ 80% of new components committed are AI-generated and approved as-is                                                                                              |
...
| Codebase shape       | Reads (does not own) the design-system monorepo; owns Figma libraries; writes Markdown docs; occasionally lands token-only PRs                                                                                                                                                                  |
...
| OS / hardware        | Linux (Debian/Ubuntu) servers + macOS workstation; manages a homelab or VPC running Gitea, LiteLLM, Authelia/Authentik/Keycloak                                                                                                                                                                              |
...
| Goals                | (1) Single deploy that serves N teams; (2) Predictable budget per-team via LiteLLM keys; (3) OAuth + audit log against IdP, no per-seat fees; (4) Prometheus metrics flowing into the existing Grafana; (5) Roll back gracefully when a model alias changes; (6) Pass security review (OWASP, CSP, sandbox)  |
...
| Goals                | (1) Treat genie as a substrate to embed in their own plugin; (2) Have stable public APIs for `list_components`, `generate_component`, `validate_design_system`; (3) Predictable semver; (4) MIT license so they can fork freely     |
...
| JTBD-02 | When the design system already has a Button, I want to refine just the hover state, so I can iterate without losing the rest of the component.                          | A, B    | M        |
...
| JTBD-04 | When I commit a component, I want validation to fail noisily if the `@dsCard` marker is missing, so the preview pane never silently loses a card.                       | A, D    | M        |
...
| JTBD-07 | When my team has a Gitea repo for the kit, I want each plan to land as a branch and each finalize as a PR, so I get free diff/review/rollback.                          | A, C    | S        |
...
| JTBD-09 | When LiteLLM is down or rate-limited, I want a friendly error and a retry-after hint, so I don't burn my own time guessing.                                              | A, D    | M        |
| JTBD-10 | When I want to swap models from Sonnet to Opus, I want a single config change, so I don't rewrite prompts.                                                              | A, C    | M        |
| JTBD-11 | When my team has 4 separate kits, I want each kit to have its own viewer port and its own LiteLLM key, so per-team budgets are enforced before tokens leave the proxy. | C       | M        |
...
| JTBD-15 | When my org uses an IdP (Authelia/Authentik/Keycloak), I want OAuth Dynamic Client Registration, so individual users don't share a bearer.                              | C       | S        |
| JTBD-16 | When I'm exploring a kit a teammate built, I want to browse `ds://components/...` resources without writing any tool call, so I can ramp quickly.                       | A, D    | S        |
...
- **Persona:** C, D
- **Narrative:** As a Platform Engineer, I want the server to speak both stdio (for local) and Streamable HTTP (for remote), so a single binary serves Claude Desktop and a Tailnet-hosted deployment.
- **Acceptance criteria:**
...
- **Persona:** A, C
- **Narrative:** As a Designer-Engineer, I want a single CLI command with `--transport`, `--port`, `--config`, `--data-dir`, `--gitea`, `--litellm` flags, so I never have to read source to launch it.
- **Acceptance criteria:**
  ...
  - Given `--gitea https://gitea.example.tld/api`, Then the Gitea backend is preferred for shared state; absent means local FS only.
  - Given `--version`, Then semver matches `package.json`.
- **Priority:** P0 · **Estimate:** S · **Milestone:** M0

### Epic E1 — Tier-0 file verbs (M1)

#### DS-010 — list_projects

- **Persona:** A, D
- **Narrative:** As a Designer-Engineer, I want to enumerate every kit project I can edit, so I can pick one without leaving the chat.
- **Acceptance criteria:**
  - Given the local FS data dir contains 3 kit directories, When `list_projects` is called, Then exactly 3 entries return with `id`, `name`, `owner`, `updatedAt`, `canEdit`.
  - Given a Gitea backend with 2 repos visible to the auth token, Then both repos appear, deduped against local FS entries by `id`.
  - Given no kits exist, Then `[]` returns (not an error).
  - Given an unreachable Gitea, Then local FS entries still return with a warning in `_meta.warnings`.
- **Priority:** P0 · **Estimate:** S · **Milestone:** M1

#### DS-011 — get_project

- **Persona:** A
- **Narrative:** As a Designer-Engineer, I want to confirm a `projectId` exists and is editable before I plan, so I never waste a `finalize_plan` round-trip.
- **Acceptance criteria:**
  - Given a valid `projectId`, Then `{ id, name, type: "PROJECT_TYPE_DESIGN_SYSTEM", canEdit }` returns.
  - Given an invalid `projectId`, Then `ERR_PROJECT_NOT_FOUND` raises with the projectId echoed.
  - Given a read-only project, Then `canEdit: false` returns and downstream `finalize_plan` is rejected pre-emptively.
- **Priority:** P0 · **Estimate:** S · **Milestone:** M1
...
- **Acceptance criteria:**
  ...
  - Given each entry, Then `path` is project-root-relative POSIX style, `size` is bytes, `hash` is `sha256-…`, `lastModified` is ISO-8601 UTC.
  - Given a kit with files outside `ui_kits/<kit>/`, Then those are filtered out.
- **Priority:** P0 · **Estimate:** S · **Milestone:** M1

#### DS-013 — get_file (256 KiB cap)
...
- **Acceptance criteria:**
  - Given `create_project({ name: "Acme Kit" })`, Then a `projectId` returns and disk shows `ui_kits/acme-kit/{manifest.json, styles.css, README.md, components/, tokens/, _vendor/}`.
  - Given a Gitea backend, Then a corresponding repo is created with the same skeleton committed to `main`.
  - Given a duplicate name, Then `ERR_PROJECT_EXISTS` raises with a suggestion to pass `--force` or pick another name.
  - Given the new project, Then a default `manifest.json` with `schemaVersion: 1` and `cards: []` is written.
- **Priority:** P0 · **Estimate:** M · **Milestone:** M1

#### DS-015 — finalize_plan with strict allow-list
...
- **Acceptance criteria:**
  - Given `finalize_plan({ projectId, writes: ["components/**", "_ds_sync.json"], deletes: [], localDir: "/tmp/ws" })`, Then a `planId` returns (uuid v7) with TTL = 15 minutes default (configurable up to 24 hours via `DS_PLAN_TTL_MIN` env var).
...
- **Acceptance criteria:**
  ...
  - Given a payload exceeding the byte cap (default 16 MiB; hard ceiling 64 MiB configurable via `DS_WRITE_BYTE_CAP` env var), Then `ERR_PAYLOAD_TOO_LARGE` raises with HTTP 500 wire-shape mapped to error code so client can halve+retry.
  ...
  - Given a `.html` write whose first line fails `/^<!--\s*@dsCard\s+group="[^"]*"[^>]*-->/`, Then `ERR_DSCARD_MISSING` raises with the failing path and the regex pattern echoed.
...
#### DS-018 — register_assets / unregister_assets (legacy)

- **Persona:** D
- **Narrative:** As an OSS Plugin Author shipping a hand-authored kit without `@dsCard` markers, I want explicit register/unregister verbs, so the manifest still builds.
- **Acceptance criteria:**
  - Given `register_assets({ planId, assets: [{ name, path, viewport, group }] })`, Then each entry appears in `manifest.json` after the next compile.
  - Given `unregister_assets({ planId, paths })`, Then each path is dropped from `manifest.json`.
  - Both verbs require a valid `planId` and reject paths outside the plan.
- **Priority:** P1 · **Estimate:** S · **Milestone:** M1

#### DS-019 — report_validate

- **Persona:** A, D
- **Narrative:** As a Designer-Engineer, I want to push validation counters back to the server, so the server can advertise validation state without re-running.
- **Acceptance criteria:**
  - Given `report_validate({ projectId, counts: { total, bad, thin, variantsIdentical, iterations } })`, Then the counts are persisted to `meta/validation.json` and surfaced on the next `get_project`.
...
- **Persona:** A, C
- **Narrative:** As a Designer-Engineer, I want the server to enforce the 5-step sequence (sentinel → writes ≤256 → deletes → re-arm sentinel → `_ds_sync.json` last), so a partial failure leaves the kit recoverable.
- **Acceptance criteria:**
  - Given any plan's first write call, Then the sentinel file `_ds_needs_recompile` is written before user content.
  - Given a `write_files` mid-sequence failure, Then `_ds_sync.json` is NOT written.
  - Given the final write, Then `_ds_sync.json` is the last byte to land on disk and contains a `writtenAt` ISO timestamp + the hashes documented in §6.6.
- **Priority:** P0 · **Estimate:** M · **Milestone:** M1

### Epic E2 — LiteLLM generation surface (M2)

#### DS-030 — generate_component (prompt-driven)

- **Persona:** A, B
- **Narrative:** As a Designer-Engineer, I want to describe a component in natural language and have the server generate the full artifact set, so I can skip boilerplate.
- **Acceptance criteria:**
  - Given `generate_component({ projectId, kit, prompt: "primary button, accent color, 3 sizes" })`, Then within P50 < 4s the server returns `{ componentName, files: [...], manifestEntry }`.
  - Given the returned files, Then exactly five are produced: `<Name>.tsx`, `<Name>.d.ts`, `<Name>.html`, `<Name>.prompt.md`, `meta.json`.
  - Given the returned `<Name>.html`, Then its first line matches the `@dsCard` regex with `group` derived from prompt context or default `"misc"`.
  - Given an optional `refImageDataUrl`, Then the image is included in the LiteLLM vision payload (Sonnet/Opus vision-capable).
  - Given `framework: "vue"`, Then files use `.vue` instead of `.tsx` and the IIFE bundle target is adjusted.
  - Given an invalid `model` alias not in LiteLLM, Then `ERR_MODEL_NOT_ROUTED` raises with the LiteLLM error message echoed.
- **Priority:** P0 · **Estimate:** L · **Milestone:** M2

#### DS-031 — refine_component (instruction + optional region rect)

- **Persona:** A, B
- **Narrative:** As a Designer-Engineer, I want to refine an existing component, optionally constrained to a region rect on the rendered card, so I can iterate without losing the rest.
- **Acceptance criteria:**
  - Given `refine_component({ projectId, componentName, instruction: "make hover state lift on shadow" })`, Then a unified diff and updated files return.
  - Given `region: { x, y, w, h }`, Then the LiteLLM prompt is annotated with the region (Sonnet/Opus interprets as "limit changes to this area") and the diff is region-scoped where possible.
...
- **Acceptance criteria:**
  - Given `list_components({ projectId })`, Then every component returns with `{ name, group, path, viewport, hash, lastModified }`.
  - Given `group: "actions"`, Then only that group returns.
  - Sort order is `group, name` ascending.
- **Priority:** P0 · **Estimate:** S · **Milestone:** M2

#### DS-033 — LiteLLM client wired to gateway

- **Persona:** C
- **Narrative:** As a Platform Engineer, I want the server to call LiteLLM via OpenAI-compatible chat-completions, so model routing, budgets, and rate-limits all reuse the gateway.
- **Acceptance criteria:**
  - Given `LITELLM_BASE_URL=https://litellm.roshangautam.com` and `LITELLM_MASTER_KEY=…`, Then `generate_component` issues a POST to `/v1/chat/completions` with `model: alias`, `response_format: { type: "json_schema" }` and `stream: true`.
  ...
  - Given a 401, Then `ERR_LITELLM_AUTH` raises immediately (no retry).
...
### Epic E3 — `@dsCard` validator + manifest compiler (M3)

#### DS-040 — @dsCard regex validator

- **Persona:** A, D
- **Narrative:** As a Designer-Engineer, I want every `.html` first-line to match the exact Anthropic regex, so previews never silently lose registration.
- **Acceptance criteria:**
  - Given a file whose first line is `<!-- @dsCard group="actions" -->`, Then validator passes.
  - Given a file whose first line is `<!-- @dsCard group="actions" viewport="480x240" -->`, Then validator passes.
  - Given any first line failing `/^<!--\s*@dsCard\s+group="[^"]*"[^>]*-->/`, Then `ERR_DSCARD_MISSING` raises with path + line content.
...
- **Persona:** A, D
- **Narrative:** As a Designer-Engineer, I want `manifest.json` to be recompiled on every `write_files` that touches a `.html`, so the viewer always renders the latest state.
- **Acceptance criteria:**
  - Given a `write_files` call that touches `.html`, Then `manifest.json` is regenerated by scanning `components/**/*.html`, extracting first-line attrs, and writing the result.
...
- **Acceptance criteria:**
  - Given `validate_design_system({ projectId })`, Then `{ dsCardMissing, thin, variantsIdentical, total, bad }` returns.
...
#### DS-043 — `_ds_sync.json` verification anchor
...
- **Acceptance criteria:**
  - Given a successful atomic write sequence, Then `_ds_sync.json` is the last file written.
  - Given its schema, Then `{ version, writtenAt, by, sourceHashes, renderHashes, verified }` is populated per §6.6.
  - Given a teammate's machine has a different `_ds_sync.json`, Then `remote-diff` produces a delta showing only files whose hash changed.
- **Priority:** P0 · **Estimate:** M · **Milestone:** M3
...
- **Acceptance criteria:**
  ...
  - Given a `manifest.json` change, Then the grid re-flows without a full reload.
...
- **Acceptance criteria:**
  - Given `file:///.../ui_kits/acme/index.html`, Then the viewer fetches `manifest.json` and renders the grid.
...
#### DS-053 — `render_preview` MCP tool with `ui://` payload

- **Persona:** A, B
- **Narrative:** As a Designer-Engineer using a host that renders MCP Apps (Claude, VS Code, ChatGPT, Cursor), I want the preview inline in chat, so I don't alt-tab.
- **Acceptance criteria:**
  - Given `render_preview({ projectId })`, Then the tool result includes both `content[].text` with the viewer URL AND `_meta.ui.resourceUri: "ui://genie/grid?projectId=…"`.
  ...
  - Given that HTML, Then it inlines `manifest.json` as `<script type="application/json" id="manifest">…</script>`, so the iframe needs no fetch.
...
#### DS-061 — OAuth 2.0 + Dynamic Client Registration

- **Persona:** C
- **Narrative:** As a Platform Engineer, I want OAuth DCR (RFC 7591) for Claude Code and Codex CLI, so each user gets their own client_id and audit log.
- **Acceptance criteria:**
  - Given a Claude Code `claude mcp add` flow, Then DCR is invoked and a per-user client_id is registered.
  ...
  - Given OAuth, Then the IdP is configurable (Authelia/Authentik/Keycloak) via `OIDC_DISCOVERY_URL`.
...
- **Persona:** C
- **Narrative:** As a Platform Engineer, I want a multi-arch container image, so I can deploy it to my TrueNAS or k8s cluster.
- **Acceptance criteria:**
  - Given `docker pull ghcr.io/roshangautam/genie:1.0`, Then both amd64 and arm64 variants resolve.
  - Given `docker run -p 8780:8780 -e LITELLM_BASE_URL=… ghcr.io/…`, Then the server starts in <2 s and `GET /healthz` returns 200.
...
- **Persona:** C, D
- **Narrative:** As a Platform Engineer, I want CI to install the server in each of 7 harnesses and run `generate → render_preview → write_files → validate`, so regressions are caught pre-release.
- **Acceptance criteria:**
...
#### DS-066 — Gitea backend (per-team kits)

- **Persona:** A, C
- **Narrative:** As a Designer-Engineer working with a team, I want each kit to be a git repo, so I get diff/PR/rollback for free.
- **Acceptance criteria:**
  - Given `--gitea https://gitea.example.tld/api` and a `GITEA_TOKEN`, Then `list_projects` enumerates editable repos.
  - Given `finalize_plan`, Then a branch `plan/<planId>` is created from `main`.
  - Given `write_files`, Then commits land on `plan/<planId>`.
  - Given a successful sequence ending in `_ds_sync.json`, Then a PR is opened from `plan/<planId>` to `main`.
  - Given the PR is merged externally, Then subsequent `list_files` reflects merged state.
- **Priority:** P1 · **Estimate:** L · **Milestone:** M5

#### DS-067 — `ds://components/{group}/{name}` resource

- **Persona:** A, D
- **Narrative:** As a Designer-Engineer in Claude Code/Cursor/VS Code, I want to `@genie:ds://components/actions/Button` as context, so the model has the latest API contract.
- **Acceptance criteria:**
  - Given a resource URI, Then the server returns the `.d.ts` content as MIME `application/typescript`.
  - Given `ds://manifest`, Then the current `manifest.json` returns.
  - Given `ds://tokens/<file>`, Then the file under `tokens/` returns.
- **Priority:** P1 · **Estimate:** M · **Milestone:** M5

#### DS-068 — MCP prompts `/genie__new-component` and `/genie__audit`

- **Persona:** A
- **Narrative:** As a Designer-Engineer in a host that surfaces MCP prompts (Claude Code, Cursor, VS Code), I want one-keystroke prompt templates.
- **Acceptance criteria:**
  - Given `/genie__new-component`, Then the prompt is parameterized with `kit`, `description`, `group?`, `framework?`.
  - Given `/genie__audit`, Then it invokes `validate_design_system` and formats the result.
- **Priority:** P1 · **Estimate:** S · **Milestone:** M5
...
- **Acceptance criteria:**
  - Given any tool call, Then a `traceId` is generated (uuid v7) and propagated to LiteLLM as `X-Trace-Id` header.
...
The server exposes **16 tools** mapped from research §3.1. All tool names
follow the shape `mcp__genie__<verb>` and respect the
`[A-Za-z0-9_-]`, ≤64-char Claude rewrite rules. Each tool description ≤2 KB.
All input/output uses JSON Schema Draft 7 — no `$ref` chains, no `anyOf`
discriminators.

| FR-ID  | Tool                       | Behavior                                                                                                       | Idempotent? | Side effects                | Errors                                                                                                                                                                  |
|--------|----------------------------|----------------------------------------------------------------------------------------------------------------|-------------|-----------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| FR-001 | `list_projects`            | Enumerates all kits visible across local FS + Gitea backends                                                   | yes         | none                        | `ERR_BACKEND_UNREACHABLE` (warning, not fatal)                                                                                                                          |
| FR-002 | `get_project`              | Returns project metadata + `canEdit`                                                                           | yes         | none                        | `ERR_PROJECT_NOT_FOUND`                                                                                                                                                 |
| FR-003 | `list_files`               | POSIX paths, sha256, ISO-8601                                                                                  | yes         | none                        | `ERR_PROJECT_NOT_FOUND`                                                                                                                                                 |
| FR-004 | `get_file`                 | 256 KiB cap, utf-8 by default, base64 for binaries                                                             | yes         | none                        | `ERR_FILE_TOO_LARGE`, `ERR_PATH_OUTSIDE_KIT`, `ERR_FILE_NOT_FOUND`                                                                                                      |
| FR-005 | `create_project`           | Spawns kit skeleton (local FS + optional Gitea repo)                                                           | no          | writes skeleton dir         | `ERR_PROJECT_EXISTS`, `ERR_BACKEND_WRITE_FAILED`                                                                                                                        |
| FR-006 | `finalize_plan`            | Locks capability grant; max 256 patterns, max 3 wildcards each                                                 | no          | persists plan               | `ERR_PROJECT_NOT_FOUND`, `ERR_PLAN_TOO_MANY_PATTERNS`, `ERR_PATTERN_TOO_WILD`                                                                                          |
| FR-007 | `write_files`              | ≤256 files/call; reads from localPath so contents never enter model context                                    | partly      | writes files atomically     | `ERR_NO_PLAN`, `ERR_PLAN_EXPIRED`, `ERR_PATH_NOT_IN_PLAN`, `ERR_TOO_MANY_FILES_PER_CALL`, `ERR_PAYLOAD_TOO_LARGE`, `ERR_DSCARD_MISSING`                                |
...
| FR-009 | `register_assets` (legacy) | Used for hand-authored projects without `@dsCard` markers                                                      | yes         | updates manifest            | `ERR_NO_PLAN`, `ERR_PATH_NOT_IN_PLAN`                                                                                                                                   |
| FR-010 | `unregister_assets` (legacy) | Legacy companion to register_assets                                                                            | yes         | updates manifest            | `ERR_NO_PLAN`, `ERR_PATH_NOT_IN_PLAN`                                                                                                                                   |
| FR-011 | `report_validate`          | Persists validation counters to `meta/validation.json`                                                         | yes         | writes one file             | `ERR_PROJECT_NOT_FOUND`                                                                                                                                                 |
| FR-012 | `generate_component`       | LiteLLM-backed generation returning 5 artifacts                                                                | no          | issues LiteLLM call         | `ERR_MODEL_NOT_ROUTED`, `ERR_GENERATION_INVALID`, `ERR_UPSTREAM_5XX`, `ERR_RATE_LIMITED`, `ERR_LITELLM_AUTH`                                                            |
| FR-013 | `refine_component`         | LiteLLM-backed refinement with optional region rect                                                            | no          | issues LiteLLM call         | as above + `ERR_COMPONENT_NOT_FOUND`                                                                                                                                    |
| FR-014 | `list_components`          | Sorted (group, name)                                                                                           | yes         | none                        | `ERR_PROJECT_NOT_FOUND`                                                                                                                                                 |
| FR-015 | `render_preview`           | Returns text URL + `ui://genie/grid?projectId=…` for MCP Apps hosts                                 | yes         | none                        | `ERR_PROJECT_NOT_FOUND`                                                                                                                                                 |
| FR-016 | `validate_design_system`   | Runs the full validator suite                                                                                  | yes         | conditional                 | `ERR_PROJECT_NOT_FOUND`                                                                                                                                                 |

**Signatures (TypeScript):**

```ts
// Tier-0 file verbs — 12-method DesignSync mirror (verbatim)
list_projects(): Array<{ id: string; name: string; owner: string; updatedAt: string; canEdit: boolean }>;
get_project(args: { projectId: string }): { id: string; name: string; type: "PROJECT_TYPE_DESIGN_SYSTEM"; canEdit: boolean };
list_files(args: { projectId: string }): Array<{ path: string; size: number; hash: string; lastModified: string }>;
get_file(args: { projectId: string; path: string }): { content: string; _meta?: { encoding: "base64" } };

create_project(args: { name: string }): { projectId: string };
finalize_plan(args: { projectId: string; writes: string[]; deletes?: string[]; localDir?: string }): { planId: string };
write_files(args: {
  planId: string;
...
register_assets(args: { planId: string; assets: Array<{ name: string; path: string; viewport: { width: number; height: number }; group: string }> }): {};
unregister_assets(args: { planId: string; paths: string[] }): {};
report_validate(args: { projectId: string; counts: { total: number; bad: number; thin: number; variantsIdentical: number; iterations: number } }): {};
list_components(args: { projectId: string; group?: string }): Array<{
  name: string; group: string; path: string; viewport: { width: number; height: number };
  hash: string; lastModified: string;
}>;

// genie-specific generation verbs
generate_component(args: {
  projectId: string; kit: string; prompt: string;
...
refine_component(args: {
  projectId: string; componentName: string; instruction: string;
  region?: { x: number; y: number; w: number; h: number };
}): { diff: string; files: Array<{ path: string; content: string }> };

render_preview(args: { projectId: string; componentName?: string; group?: string }): {
  content: Array<{ type: "text"; text: string }>;
  _meta: { ui: { resourceUri: string } };
};

validate_design_system(args: { projectId: string; planId?: string }): {
  dsCardMissing: string[]; thin: string[]; variantsIdentical: string[]; total: number; bad: number;
};
```

**FR-017 — Atomic write contract.** Every `write_files` call follows the
5-step sequence: (a) `_ds_needs_recompile` sentinel first (only on first
call of a plan), (b) content writes ≤ 256/call, (c) deletes, (d) re-arm
sentinel, (e) `_ds_sync.json` last (only on the call marked `_meta.final: true`).

**FR-018 — Plan TTL.** Plans expire after a configurable TTL — 15 minutes
default, configurable up to 24 hours via `DS_PLAN_TTL_MIN` env var. v1 has
...
`<data-dir>/_state/plans.sqlite` so a server restart does not lose
in-flight plans. SQLite chosen for zero-dependency embed.
...
**FR-021 — `ds://components/{group}/{name}`.** Returns the component's
`<Name>.d.ts` file as MIME `application/typescript`. Used by host
autocomplete in Claude Code (`@genie:ds://components/actions/Button`),
Cursor, VS Code.

**FR-022 — `ds://manifest`.** Returns the current `manifest.json` for the
default project (or the project named in a `?projectId=` query string).
MIME `application/json`.

**FR-023 — `ds://tokens/{file}`.** Returns any file under the kit's
...
**FR-025 — `/genie__new-component`.** Parameters:
`{ kit: string, description: string, group?: string, framework?: "react"|"vue"|"html" }`.
Expands to a prompt that calls `generate_component` then `render_preview`.

**FR-026 — `/genie__audit`.** Parameters:
`{ projectId: string }`. Calls `validate_design_system` and formats the
result as a Markdown table grouped by violation type.

### 6.4 MCP-UI Apps payload (`ui://genie/grid`)

**FR-027 — Resource URI.** `ui://genie/grid?projectId=<id>&componentName=<opt>&group=<opt>`.

**FR-028 — MIME.** `text/html;profile=mcp-app` exactly, per the MCP Apps
spec (targeted 2026-01-26 — see INDEX honest-uncertainty #4).

**FR-029 — Tool linkage.** `render_preview` returns
`_meta.ui.resourceUri = "ui://genie/grid?projectId=…"`. Hosts
that support MCP Apps fetch the resource and embed it.

**FR-030 — Self-contained HTML.** The returned HTML inlines `manifest.json`
...
### 6.5 LiteLLM gateway integration

**FR-040 — Model alias.** Default model alias `design-default` (mapped in
LiteLLM `config.yaml` to `anthropic/claude-sonnet-4-6`). Additional aliases
`design-best` (Opus 4.8) and `design-local` (Qwen3-Coder via Ollama).
*Aliases shown are placeholders — confirm against
https://litellm.roshangautam.com/v1/models. Marketed Opus 4.7 resolves to
`anthropic/claude-opus-4-8`.*

**FR-041 — Endpoint.** `LITELLM_BASE_URL` (default `https://litellm.roshangautam.com`)
+ `/v1/chat/completions`.

**FR-042 — Auth.** `Authorization: Bearer <LITELLM_MASTER_KEY>`. The
master key lives in `~/.shellprivatevars`; never logged.
...
Props signatures, the 5-file artifact contract, the `@dsCard` regex
requirement.
...
Server validates the LiteLLM response against this schema, attempts one
self-repair retry, then raises `ERR_GENERATION_INVALID`.

**FR-045 — Streaming.** `stream: true` on the LiteLLM POST. Server
...
the LiteLLM request; the same id appears in logs and metric labels.

**FR-048 — Budget headers.** `x-litellm-key` and `x-litellm-team` headers
are passed through when present so per-team budgets are charged.

### 6.6 Local component store schema

The on-disk layout mirrors the bundled `design-sync` skill verbatim so a
kit produced by genie can round-trip into a real Claude Design project.

```
ui_kits/<kit-name>/
├── _ds_bundle.js              # IIFE; window.<global>.<Component> exports
├── _ds_bundle.css             # bundle styles
├── styles.css                 # import-closure root previews consume
├── README.md                  # kit docs (human)
├── manifest.json              # client-side index (server compiles)
├── _ds_needs_recompile        # sentinel: { "by": "genie" }
├── _ds_sync.json              # verification anchor (always last)
...
│   └── chrome.css
├── guidelines/                # design-system guidelines (Markdown)
...
└── components/
    └── <group>/<Name>/
        ...
        ├── <Name>.html        # preview; FIRST LINE = @dsCard marker
        └── meta.json          # per-component metadata
```

**FR-050 — manifest.json schema** (`schemaVersion: 1`):
...
`/^<!--\s*@dsCard\s+group="[^"]*"[^>]*-->/`. Optional attributes after
...
**FR-055 — `_ds_needs_recompile` sentinel** contents: `{ "by": "genie" }`.
Cleared by the server-side self-check after manifest recompile.

**FR-056 — `_ds_bundle.js` header.** First line:
`/* @ds-bundle: { "schemaVersion": 1, "global": "AcmeKit" } */`. The
bundle is an IIFE assigning every component to `window.AcmeKit.<Component>`.

**FR-057 — Reserved file names.** The following names are reserved at the
kit root and may not be overwritten outside the canonical flow:
`manifest.json`, `_ds_sync.json`, `_ds_needs_recompile`, `_ds_bundle.js`,
`_ds_bundle.css`, `styles.css`, `index.html`.
...
### 6.7 The `@dsCard` regex contract

**FR-060 — Exact regex.** `/^<!--\s*@dsCard\s+group="[^"]*"[^>]*-->/`.
...
(2) `validate_design_system`, (3) the manifest compiler (which skips
non-matching files with a warning rather than aborting).

**FR-064 — Error code.** `ERR_DSCARD_MISSING` (also surfaces as
`[DSCARD_MISSING]` in human-formatted errors, matching the bundled
skill's convention).

**FR-065 — Error message shape.**
`[DSCARD_MISSING] <path>: first line isn't a \`<!-- @dsCard group="…" -->\` comment`.

### 6.8 Atomic write sequence

**FR-070 — Step 1 (sentinel first).** On the first `write_files` call of
a new plan, the server writes `_ds_needs_recompile` BEFORE any
user-content file lands. Reason: the sentinel fences genie's own
manifest/copy machinery, mirroring Anthropic's server-side behavior.
...
**FR-074 — Step 5 (`_ds_sync.json` last).** `_ds_sync.json` is written as
...
fails, the sequence STOPS without writing `_ds_sync.json`. Client is
...
watching `components/**/preview.html` and `manifest.json`. HMR pushes
...
On a `manifest.json` save → grid re-flows without full page reload.
...
**FR-086 — File watching exclusions.** `.git/`, `_state/`, `node_modules/`
excluded.
...
toggle; per-card iframes are unaffected (design system owns its theme).
...
| NFR-002 | Generation Time-To-First-Token < 800 ms via LiteLLM                                                | Prom histogram `genie_generate_ttft_seconds`                                 |
| NFR-003 | Generation end-to-end P50 < 4 s for typical components (≤ 200 LOC), P95 < 9 s                    | Prom histogram `genie_tool_latency_seconds{tool="generate_component"}`        |
...
| NFR-006 | `validate_design_system` runs in ≤ 5 s for a 100-component kit                                     | bench harness                                                                      |
...
| NFR-013 | All secrets via env vars (`LITELLM_MASTER_KEY`, `GENIE_TOKEN`, `GITEA_TOKEN`); never accept secrets via tool params, never write them to logs or disk                                    |
...
| NFR-015 | Path traversal protection on every file verb (`get_file`, `write_files`, `delete_files`): resolve and confirm prefix matches kit root; reject `..` segments before normalization              |
...
| NFR-ID  | Requirement                                                                                                                            |
|---------|----------------------------------------------------------------------------------------------------------------------------------------|
| NFR-030 | Graceful 429 from LiteLLM → respect `Retry-After` header; do NOT auto-retry; surface `ERR_RATE_LIMITED` to the caller with the retry-after value |
| NFR-031 | Graceful 5xx from LiteLLM → 3 retries with exponential backoff; on exhaustion, `ERR_UPSTREAM_5XX`                                       |
| NFR-032 | `finalize_plan` and `write_files` are idempotent for identical `planId` + identical payload: re-calls do not duplicate writes          |
| NFR-033 | Plan storage durable across restart (SQLite under `<data-dir>/_state/`)                                                                |
| NFR-034 | Healthcheck `GET /healthz` returns 200 within 100 ms when server is up                                                                  |
| NFR-035 | Readiness `GET /readyz` returns 200 only after LiteLLM is reachable (or 503 with a "litellm unreachable" body)                         |
...
| NFR-ID  | Requirement                                                                                                                                                                    |
|---------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| NFR-060 | Prometheus exposition at `GET /metrics` with: `genie_tool_calls_total`, `genie_tool_latency_seconds`, `genie_generate_tokens_total`, `genie_generate_ttft_seconds`, `genie_validation_failures_total`, `genie_plan_active`, `genie_write_files_throughput`, `genie_litellm_errors_total`, `genie_litellm_retries_total`, `genie_oauth_logins_total`, `genie_oauth_failures_total`, `genie_resource_fetches_total` |
| NFR-061 | Structured JSON logs to stdout — one event per line with `{ts, level, traceId, tool, msg, durationMs, …}`                                                                        |
| NFR-062 | Trace IDs (uuid v7) generated on every tool call, propagated to LiteLLM as `X-Trace-Id`                                                                                          |
| NFR-063 | Log level configurable via `LOG_LEVEL` env: debug, info, warn, error                                                                                                              |
...
Numbered. Each flow uses a sequence-style step list. Where harness UI
specifics matter, the flow is annotated for that harness.

### Flow 1 — First-time install in Claude Code

1. Designer-Engineer Priya opens Claude Code and types `/help mcp`.
2. She runs `claude mcp add --transport http genie https://genie.tld/mcp`.
3. Claude Code's CLI initiates an OAuth Dynamic Client Registration flow.
4. The CLI opens a browser to the IdP (Authelia) consent screen.
5. Priya approves the `read:components write:components` scopes.
6. The IdP redirects to the local callback; Claude Code stores the bearer
   token in `~/.claude/credentials.json` (chmod 600).
7. `claude mcp list` shows `genie   ✓ ready (16 tools)`.
8. She runs `/mcp__genie__list_projects` (autocomplete via `/mcp`).
9. Result: an empty list. She types "create a new kit called acme-kit".
10. Claude invokes `create_project({name:"acme-kit"})` → `{projectId:"acme-kit"}`.
11. Priya can now reference `@genie:ds://manifest` to view the
    kit's compiled card index.

**Failure modes:**
- OAuth times out → `ERR_OAUTH_TIMEOUT`; Claude Code re-prompts after 30 s.
- Token revoked → 401 on first call; Claude Code re-runs DCR.

### Flow 2 — Generate a new Button component from a prompt

1. Priya types: "generate a Primary Button component in the actions group
   with three sizes and a disabled state."
2. Claude reasons → tool call:
   `generate_component({projectId:"acme-kit", kit:"acme-kit", prompt:"…", group:"actions"})`.
3. Server validates the prompt is non-empty and the projectId exists.
4. Server constructs LiteLLM request: model `design-default`,
   system prompt loaded from `prompts/system.md` augmented with the kit's
   tokens summary + existing components' Props.
5. LiteLLM streams chunks. Server forwards `notifications/progress` events
   so Claude shows partial output.
6. Server validates the final JSON against `COMPONENT_SCHEMA`.
7. Server runs `finalize_plan({projectId, writes:["components/actions/Button/**", "_ds_sync.json", "_ds_needs_recompile", "manifest.json"], localDir:"/tmp/genie-staging/abc123"})` → `planId`.
8. Server writes the 5 artifacts to `localDir`.
9. Server calls `write_files` with the 5 paths.
10. Server calls `write_files` with `_meta.final: true` for the anchor +
    manifest.
11. Tool returns `{componentName:"Button", files:[…], manifestEntry:{…}}`.
12. Claude pretty-prints the result and offers to run `render_preview`.

**Failure modes:**
- LiteLLM 429 → `ERR_RATE_LIMITED`; Claude tells Priya the retry-after.
- Schema repair fails → `ERR_GENERATION_INVALID`; Claude offers to retry
  with a tighter prompt.
- `@dsCard` missing → `ERR_DSCARD_MISSING`; Claude amends the prompt
  with an explicit `@dsCard` line and retries.

### Flow 3 — Refine an existing component via region rect

1. Priya is looking at the live viewer at `http://localhost:5173`.
2. She clicks "Refine" on the Button card → the viewer's iframe overlay
   captures her drag-rect: `{x:120, y:80, w:200, h:60}` (the hover state).
3. The overlay posts `{type:'refine', componentName:'Button', region:{...},
   instruction:'lift the shadow on hover'}` to the parent.
4. The viewer relays this to Claude Code via the MCP-Apps `ui/message`
   bridge.
5. Claude invokes `refine_component({projectId, componentName:"Button",
   instruction:"lift the shadow on hover", region:{x:120,y:80,w:200,h:60}})`.
6. Server constructs LiteLLM prompt: includes existing `<Name>.tsx`, the
   instruction, and the region rect ("limit changes to the area at
   x=120,y=80,w=200,h=60 in the rendered preview").
7. LiteLLM returns a unified diff + updated files.
8. Server validates `@dsCard` regex still passes on the updated `.html`.
9. Server runs the atomic 5-step write sequence.
10. Viewer's chokidar watcher detects the `preview.html` change → pushes
    `{type:'refresh', id:'button-primary'}` over WebSocket.
11. The iframe reloads itself within 100 ms — Priya sees the new hover.

### Flow 4 — Open the preview pane from any harness

| Harness         | What happens                                                                                                                            |
|-----------------|-----------------------------------------------------------------------------------------------------------------------------------------|
| Claude Code     | `render_preview` returns text URL + `_meta.ui.resourceUri`. Claude renders the URL as a link; if `ui://` is supported (TBD), inline.    |
| Claude Desktop  | Text URL printed; user clicks; default browser opens viewer at `http://localhost:5173`. Also `ui://` is supported per MCP Apps spec.    |
| Codex CLI       | Text URL printed in terminal; user copies to browser. No `ui://` rendering.                                                              |
| VS Code Copilot | `ui://` rendered inline in the chat panel (post-Jan 2026 Stable). Text URL also shown.                                                  |
| Cursor          | Text URL shown; Cursor's Apps extension renders the `ui://` inline.                                                                      |
| Cline           | Text URL only; user clicks.                                                                                                              |
| Continue        | Text URL only; user clicks.                                                                                                              |

All paths converge on the same `index.html` and same `manifest.json`.

### Flow 5 — Sync a kit to a remote Gitea repo

1. Devika (Platform Engineer) configures the server with `--gitea https://gitea.example.tld/api` and `GITEA_TOKEN`.
2. Priya runs `create_project({name:"acme-kit"})`.
3. Server creates `acme-kit` repo on Gitea with initial commit on `main`.
4. Priya runs `generate_component(...)` which kicks off Flow 2.
...
8. Priya's teammate reviews + merges the PR in Gitea's UI.
9. Subsequent `list_files({projectId})` reflects the merged state.

### Flow 6 — Audit a kit for missing @dsCard markers

1. Priya types `/genie__audit acme-kit`.
2. Claude invokes the prompt template, which fans out:
   `validate_design_system({projectId:"acme-kit"})`.
3. Server walks `components/**/*.html`, applies the regex to first line,
   collects any failures into `dsCardMissing[]`.
...
7. The prompt template formats the report as a Markdown table grouped by
   category.
8. Priya sees: 2 missing `@dsCard`, 1 thin component, 0 duplicates.
9. She types: "fix the missing @dsCard markers in
   `components/feedback/Toast/Toast.html` and
   `components/inputs/Switch/Switch.html`."
10. Claude amends the first line of each file and re-runs the audit.

### Flow 7 — Upgrade from one model alias to another

1. Devika edits LiteLLM `config.yaml`: changes `design-default` from
   `anthropic/claude-sonnet-4-6` to `anthropic/claude-opus-4-8`.
2. She runs `litellm proxy reload` (no MCP server restart needed).
3. Priya's next `generate_component` call routes through Opus.
4. Latency, cost, and quality differ; she can override per-call with
   `model: "design-default"` (or set the alias back).
5. Roll back: revert the YAML, reload LiteLLM. Zero server change.

**Per-call override:** Priya can pass `model: "design-best"` or
`model: "design-local"` (Qwen3-Coder via Ollama) for any single call.

### Flow 8 — Handle a LiteLLM outage

1. LiteLLM goes down (network blip, container restart).
2. Priya runs `generate_component(...)`.
...
6. Claude pretty-prints: "LiteLLM gateway unreachable. Underlying:
   `ECONNREFUSED 100.81.124.86:4000`. Try again or check `litellm proxy
   status`."
7. Priya checks `https://litellm.roshangautam.com/health` (returns 503).
8. She SSHs to TrueNAS, restarts the LiteLLM container.
9. `GET /readyz` on the MCP server returns 503 ("litellm unreachable")
   until LiteLLM is back; then 200.
10. Priya retries `generate_component(...)` and it succeeds.
...
|--------|-------------------------------------------------------------------------------|------------------------------------------------------------------------------------|----------------------------------|--------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------|
| EC-001 | `planId` expired (older than configured TTL; default 15 min, configurable up to 24h via `DS_PLAN_TTL_MIN`) | Reject `write_files`; clear plan from KV                                            | `ERR_PLAN_EXPIRED`               | "Plan abc123 expired (TTL 15 min). Re-finalize." (N reflects configured value)                                       | Client calls `finalize_plan` again with same writes/deletes; receives a new planId.    |
| EC-002 | File path outside the plan's `writes` allow-list                              | Reject the entire `write_files` call (no partial writes)                            | `ERR_PATH_NOT_IN_PLAN`           | "Path components/widgets/Foo.html not in plan abc123's writes."                                                    | Re-finalize the plan with the missing pattern included.                                |
...
| EC-004 | Sentinel file race: two clients write `_ds_needs_recompile` concurrently     | Server uses POSIX `O_EXCL` semantics; second writer becomes a touch (mtime bump)   | (no error)                       | n/a                                                                                                                | n/a                                                                                     |
| EC-005 | LiteLLM 401 (invalid master key)                                              | No retry; surface immediately                                                       | `ERR_LITELLM_AUTH`               | "LiteLLM returned 401 — master key invalid or expired."                                                            | Devika rotates `LITELLM_MASTER_KEY` in env; restart server.                            |
| EC-006 | LiteLLM 429 (rate limit)                                                      | Respect `Retry-After` header; do NOT auto-retry; surface `ERR_RATE_LIMITED` to caller with retry-after value | `ERR_RATE_LIMITED`               | "LiteLLM rate-limited (retry in 12 s)."                                                                            | Wait or upgrade the LiteLLM team's budget.                                              |
| EC-007 | LiteLLM 5xx                                                                   | 3 retries exponential backoff; then surface                                         | `ERR_UPSTREAM_5XX`               | "LiteLLM returned 502 after 3 retries: \\"upstream timeout\\"."                                                    | Restart LiteLLM or check homelab connectivity.                                          |
| EC-008 | Harness disconnects mid-stream (Claude Code crash during generation)         | Server completes the LiteLLM call and discards the response (no orphan files)      | (no error to client)             | n/a                                                                                                                | Client reconnects and retries the tool call.                                            |
...
| EC-011 | Gitea unreachable                                                             | `list_projects` returns local FS entries with a warning in `_meta.warnings`        | `ERR_BACKEND_UNREACHABLE` (warn) | "Gitea https://gitea.example.tld/api unreachable; showing local kits only."                                       | Devika fixes Gitea; subsequent calls auto-recover.                                      |
| EC-012 | Malformed `@dsCard` line                                                      | Validator rejects on first failing file; aborts entire `write_files`                | `ERR_DSCARD_MISSING`             | "[DSCARD_MISSING] components/actions/Button/Button.html: first line isn't a @dsCard comment."                      | Author prepends the correct first line.                                                 |
| EC-013 | `manifest.json` schemaVersion mismatch (v1 server sees v2 manifest on disk)  | Server treats as upgrade; reads as v1 with warnings; v2-only fields ignored        | (no error)                       | "manifest.json schemaVersion=2 read by v1 server; some fields ignored."                                            | Upgrade server.                                                                         |
...
| EC-015 | `_ds_sync.json` written mid-sequence (client violates atomic contract)        | Server detects (writes outside the canonical step 5); raises                       | `ERR_ANCHOR_OUT_OF_ORDER`        | "_ds_sync.json may only be written as the terminal step (_meta.final:true)."                                      | Client re-issues with correct ordering.                                                 |
| EC-016 | `localDir` outside server's allowed paths                                      | `finalize_plan` rejects                                                            | `ERR_LOCALDIR_FORBIDDEN`         | "localDir /etc not under allowed roots."                                                                            | Use a directory under `--data-dir`.                                                     |
| EC-017 | `get_file` on a 300 KiB file                                                  | Reject; do not stream partial                                                       | `ERR_FILE_TOO_LARGE`             | "File 300000 bytes > 262144 byte cap."                                                                              | Use `list_files` for metadata only; fetch via git for full content.                     |
| EC-018 | Binary file in `get_file`                                                     | Return base64 with `_meta.encoding: "base64"`                                       | (no error)                       | n/a                                                                                                                | Client decodes.                                                                         |
| EC-019 | `generate_component` returns a 4-file response (one missing)                  | Schema repair retry; if still 4, raise                                              | `ERR_GENERATION_INVALID`         | "Generation missing required file <Name>.html (got 4 of 5)."                                                       | Re-prompt with stricter system instructions.                                            |
| EC-020 | Plan SQLite corruption                                                        | Server logs corruption; recreates empty DB; warns                                   | `ERR_PLAN_STATE_RESET`           | "Plan state DB corrupted; reset. Active plans lost."                                                                | Re-finalize active plans.                                                               |
...
| EC-022 | Two simultaneous `finalize_plan` for same projectId                          | Both succeed; both planIds valid; writes to overlapping paths are last-write-wins  | (no error)                       | n/a                                                                                                                | Client serializes its own plans.                                                        |
...
| EC-024 | Streaming response disconnect mid-token                                       | Server treats as 5xx; retries 3x                                                    | `ERR_UPSTREAM_5XX`                | "LiteLLM stream disconnected."                                                                                     | Retry the call.                                                                         |
...
| EC-026 | Two kits with same `name` on disk                                             | `create_project` rejects                                                            | `ERR_PROJECT_EXISTS`             | "Kit 'acme' already exists. Use --force or pick another name."                                                     | Rename or pass `--force`.                                                               |
| EC-027 | `refine_component` with non-existent componentName                            | Reject                                                                              | `ERR_COMPONENT_NOT_FOUND`        | "Component 'Foo' not found in projectId 'acme-kit'."                                                                | Use `list_components` to confirm names.                                                 |
| EC-028 | `register_assets` with viewport `{width:0,height:0}`                          | Reject                                                                              | `ERR_INVALID_VIEWPORT`           | "viewport.width and viewport.height must be ≥ 1."                                                                  | Provide a non-zero viewport.                                                            |
| EC-029 | `write_files` exceeding payload byte cap (default 16 MiB; hard ceiling 64 MiB configurable via `DS_WRITE_BYTE_CAP` env var) | Reject                                                                              | `ERR_PAYLOAD_TOO_LARGE`          | "Payload 18 MiB > 16 MiB cap; halve chunk size."                                                                    | Client halves and retries (halve-and-retry recovery: on HTTP 500 from `write_files`, halve the file batch and retry; abort after 3 halvings). |
| EC-030 | Server clock skew vs LiteLLM (> 5 min)                                        | OAuth signing may reject                                                            | `ERR_CLOCK_SKEW`                 | "System clock differs from LiteLLM by > 5 min; sync via NTP."                                                       | `sudo ntpdate -u pool.ntp.org`.                                                          |
...
|--------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Canvas-side generation pane (a la `claude.ai/design` UI)      | Claude's pane is hosted-only with an undocumented prompt loop; building one is open-ended R&D unrelated to portability                                       |
...
| SAML/SCIM enterprise SSO                                      | OIDC covers Authelia/Authentik/Keycloak; SAML deferred until enterprise demand surfaces                                                                     |
| Mobile (iOS/Android) clients of the viewer                    | Desktop browsers only in v1; mobile is a v2 question                                                                                                        |
| Cost/billing dashboards                                       | LiteLLM's own dashboards cover this; we expose Prom metrics but no UI                                                                                       |
...
| Auto-rollback on validation failure                           | Plan-based atomicity means validation failures simply abort `_ds_sync.json`; the kit is recoverable but not auto-rolled-back                                |
...
| Direct LLM provider calls (skipping LiteLLM)                  | LiteLLM is mandatory; gives uniform retry, budget, audit; bypassing it would defeat the platform-engineer value prop                                         |
| Server-Sent Events transport                                  | Deprecated in Claude Code; only "legacy" in Cline; not worth the complexity (research §4)                                                                   |
| Bundled IdP (Authelia/Keycloak)                               | Users bring their own IdP; we document the OIDC discovery URL hook                                                                                          |

## 11. Release plan

Dates are relative to T+0 (project kickoff). Engineering team of 2.

| Release | Target date    | Scope                                                                                                                                    | Exit criteria                                                                                                                                                |
|---------|----------------|------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------|
| v0.1    | T + 2 weeks    | M0 (scaffold + dual transport + CLI) + M1 (Tier-0 file verbs)                                                                            | Internal dogfood: Priya can `create_project`, `finalize_plan`, `write_files`, `delete_files`, `list_files`, `get_file` from Claude Code stdio                |
| v0.5    | T + 5 weeks    | + M2 (LiteLLM generation surface) + M3 (`@dsCard` validator + manifest compiler) — single-harness Claude Code stdio focus               | Alpha: external testers can generate a Button + Card kit end-to-end in Claude Code; @dsCard validation gating works; LiteLLM 429/5xx handled                 |
...
1. **Default model alias choice — sonnet-4-6 vs opus-4-8?** Sonnet is
   faster/cheaper; Opus is higher quality. Pick one as the default the
   user sees. *Tag: needs a decision.*
2. **`_ds_sync.json` "final" semantics — implicit (server tracks plan
   completion) or explicit (`_meta.final: true` flag on terminal
   write)?** v1 prefers explicit, but Anthropic's behavior is unclear.
   *Tag: needs technical spike.*
3. **Should `register_assets` / `unregister_assets` ship in v1, or be
   deferred to v1.x?** They're legacy in Anthropic's tool. Excluding them
   simplifies the API but breaks hand-authored kits. *Tag: needs a
   decision.*
4. **Should the viewer ship as a separate package (`@genie/viewer`)
   or be bundled into the main npm package?** Separate gives users the
   option to skip Vite/chokidar; bundled is one-command. *Tag: needs
   decision.*
5. **Should we adopt the "Anthropic Labs" branding (now killed per
   ...
   Affects positioning copy on README. *Tag: needs a decision.*
6. **Plan TTL default of 15 min — is that long enough for designer-driven
   flows where the user pauses to read?** TTL is now configurable up to
   24 hours via `DS_PLAN_TTL_MIN` env var; question is whether to bump
   the default and/or add a `renew_plan` verb. *Tag: needs user research.*
7. **Per-team budget enforcement — should the server short-circuit the
   call when LiteLLM's reported budget is exhausted, or always let
   LiteLLM be the authority?** The latter is simpler; the former gives
   faster failure. *Tag: needs technical spike.*
8. **Should the viewer expose a "share" link that bakes the kit's
   `manifest.json` into a single static URL on a CDN, so designers can
   drop a link in Slack without `localhost`?** *Tag: needs user research.*
9. **Auth fallback for OSS users without an IdP — accept a simple
   pre-shared `GENIE_TOKEN` as the only auth path?** v1 already
   does this; question is whether we deprecate it in v2. *Tag: needs
   decision.*
10. **Should `generate_component` accept multi-file outputs (a Button
    plus its CSS module + storybook story) for v1, or restrict to the
    canonical 5-file output?** Sticking with 5 is portable; flexible is
    powerful. *Tag: needs technical spike.*
11. **MCP-UI `ui://` rich rendering in Claude Code — verified or
    aspirational?** Per research uncertainty #3. If unverified at launch,
    we should ship the text URL fallback by default and add a `--ui`
    flag. *Tag: needs technical spike.*
12. **Should we ship a Claude Code plugin wrapping the server (so
    `claude /plugin install genie` works) or only the
    plain MCP server?** *Tag: needs a decision.*
